...
with open(os.path.join(os.path.dirname(__file__), "content.md")) as f:
    content = markdown.markdown(f.read().strip(), extensions=["codehilite"]).replace("&gt;&gt;&gt;", "<span class=\"int\">&gt;&gt;&gt;</span>").replace("{{","<span class=\"output\">").replace("}}","</span>").replace("...", "<span class=\"int\">...</span>")
